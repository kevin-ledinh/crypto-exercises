...
int main() {
	...
	if( myfile.is_open() )
	{
		char message[ 1024 + SHA256::DIGESTSIZE ] = {0};
		byte signature[ SHA256::DIGESTSIZE ];
		...
		myfile.read(message, LastBlockSize); // break here OMG
		cout << "Current file pos: " << myfile.tellg() << endl;

		GetSHA256(sha, message , LastBlockSize , signature , SHA256::DIGESTSIZE );
		...
		for(int i = 1; i < NumberOfBlocks; i++)
		{
			...
			currentRdPos = FileSize - ( BlockSize * i );
			myfile.seekg (currentRdPos, ios::beg);
			myfile.read(message, currentRdPos);

			// Append the hash of the last block to the current block
			GetSHA256(sha, message , BlockSize , signature , SHA256::DIGESTSIZE );
